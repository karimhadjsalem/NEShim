...
/// <c>Controls.SliderControl.ComputeGeometry</c> so the geometry math is unit-testable
...
/// <param name="ValueRect">Destination rect for the formatted value text, left-aligned within it
/// so the digits sit immediately after the bar rather than pinned to the row's own right edge
/// (see SliderControl.Draw's comment on why Near, not Far, alignment is used here).</param>
