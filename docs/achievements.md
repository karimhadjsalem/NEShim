...
1. Choose unused addresses in zero page, e.g. `0x00E0`–`0x00FF`.
2. In your game code, write `0x01` to `0x00E0` when the player reaches the first achievement, `0x01` to `0x00E1` for the second, etc.
...
- **`StatsReady` guard:** Achievements are suppressed until Steam returns the initial stats snapshot (triggered by `RequestCurrentStats()`). This prevents calling `SetAchievement` before Steam is ready, which would silently fail.
