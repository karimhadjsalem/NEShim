...
**Device loss recovery:** both renderers implement `IFrameRenderer.DeviceLost`, and `NEShimApp.OnRendererDeviceLost` (subscribed once, at both initial renderer creation and inside its own recovery path) handles either the same way — platform-agnostic: setting `PauseReasons.DeviceLost`, disposing both renderer and overlay renderer, recreating them via `RendererFactory.Create`/`OverlayRendererFactory.Create` (calling `_renderer.SetMenuSceneProvider(this)` again on the new renderer), then clearing the pause reason. The two renderers detect loss differently, reflecting what each platform's API actually exposes: `D3D11Renderer.PresentAndCheckResult` fires `DeviceLost` immediately on the swap chain Present call's HRESULT being `DXGI_ERROR_DEVICE_REMOVED`/`DXGI_ERROR_DEVICE_RESET` — structured, unambiguous codes, so no debouncing is needed. `SDL3HwRenderer.PresentAndCheckDeviceLost` has no such structured signal available (`SDL.RenderPresent` returns only a bool plus a free-form `SDL.GetError()` string, which can't reliably distinguish a genuinely lost Vulkan/GPU device from a benign, self-recovering hiccup like a resize-driven swapchain race) — it requires `DeviceLostFailureThreshold` (3) *consecutive* failed presents before firing `DeviceLost`, and resets that counter on `Resize()` so a resize's own transient failures don't accumulate toward a false trigger.

`SteamOverlayRenderer` must be initialised **after** `SetWindowMode` so the swap chain dimensions match the final window size. `D3D11Renderer` is constructed immediately after. An `SDL3WindowHost.Resized` event calls `D3D11Renderer.Resize()` which calls `ResizeBuffers` and recreates the RTV.
...
- **Filmstrip layout**: `GameCarouselRenderer.ComputeSlotGameIndices(selectedIndex, gameCount, visibleSlotCount)` maps carousel state to which game occupies each visible slot via plain modulo wraparound, so a library smaller than `visibleSlotCount` naturally repeats games to fill the strip — reading correctly as a carousel, since the same neighbor really is reachable from either direction. The one exception is the centered/selected game itself: it never repeats in a non-center slot (that slot is `NoGame` instead, skipped by both `DrawStatic` and `DrawSliding`) — e.g. a 1-game library shows only the centered slot with both neighbors empty (nothing to wrap to, and `GameCarouselScreen.MoveNext`/`MovePrevious` no-op entirely rather than playing a slide animation that lands back on the same tile), and a 2-game library shows the other game on both the immediate left and right of the centered slot, with the next slot out on each side empty (wrapping further would land back on the centered game).
- **Owned resources**: `GameCarouselScreen` is `IDisposable` — it owns per-game thumbnail surfaces (loaded once at construction, resolved per game via `MainMenuScreen.ResolveAssetPath(game.ThumbnailPath, GameContext.ForGame(...))`; a missing file is not an error, the renderer falls back to a procedurally-drawn placeholder card) and an optional `AnimatedImagePlayer` background (`NEShim/Rendering/AnimatedImagePlayer.cs` — plays back a decoded GIF/WEBP/APNG frame sequence, or a single static image, based on wall-clock elapsed time; backed by `SdlSurfaceLoader.LoadAnimationFromFile`, a thin marshalling wrapper around SDL3_image's `Image.LoadAnimation`/`FreeAnimation`, already available via the existing `SDL3-CS.*.Image` packages with no new dependency).
- **Background rendering**: drawn via `SDL3PaintContext.BlitSurfaceUncached` rather than the cached `BlitSurface`/`BlitSurfaceAlpha` path thumbnails use — a background frame's surface pointer can mean different pixel content at different points in playback, so caching it by pointer would freeze whichever content was uploaded first; `RenderSurface`'s `_textureCache` is only safe for genuinely static, one-shot surfaces like thumbnails and box art. Since the background is therefore re-uploaded every frame, `AnimatedImagePlayer.LoadFromFile`'s optional `maxWidth`/`maxHeight` (passed as the constructor's `viewportWidth`/`viewportHeight`, sourced from `SDL3WindowHost.ClientWidth`/`ClientHeight`) downscales every decoded frame at load time to the actual window size, never upscaling — this bounds the per-frame re-upload cost against the current render target rather than a fixed constant, since (unlike a thumbnail tile) the background can legitimately need to fill anything from a 720p window to a 4K one.
- **Animation timing**: Left/Right selection changes and the Up-to-flip-description card both animate via progress properties (`SlideProgress`, `FlipProgress`) computed from wall-clock elapsed time on every access — the same convention as `LogoScreen.CurrentAlpha` — backed by pure, unit-tested `ComputeSlideProgress`/`ComputeFlipProgress`/`ComputeFlipVisual`/`ComputeBoxArtRect` functions mirroring `LogoRenderer.ComputeDisplayRect`.
...
- **Signing keypair**: run `pub-utils --gen-keypair` once per game. Set the printed public key as `achievementPublicKey` in `config.json` (pre-built release) or in `AchievementSigner.EmbeddedPublicKeyBase64` and rebuild (source build). Store the private key outside source control. Achievements are disabled until a key is configured — there is no shipped default.
- **Achievements**: edit `achievements.json`, then run `pub-utils --key-file private_key.txt achievements.json` to stamp ECDSA-P256 signatures. Re-seal any time a definition changes.
...
- **Steamworks native library**: not included in the repository. After `dotnet publish`, copy the matching library from the [Steamworks.NET GitHub release zip](https://github.com/rlabrecque/Steamworks.NET/releases) into the output directory alongside the exe: `steam_api64.dll` for Windows (`win-x64`). For Linux (`linux-x64`), copy the zip's `libsteam_api.so` but **rename it to `libsteam_api64.so`** in the output directory — see the "Linux native lib naming gotcha" note above; the wrapper's `DllImport` resolves to `libsteam_api64.so` and silently fails `SteamAPI_Init` if the file keeps its zip-default name. Use the copy bundled with the wrapper (matched version); do not pull it from the Steamworks SDK partner dashboard separately.
...
- **DLC ownership anti-tamper — signed tier (optional, real tamper-proofing, recommended for a real commercial DLC release)**: run `pub-utils --gen-keypair` to generate a **fresh keypair dedicated to this purpose — do NOT reuse the achievement-signing keypair.** The two protect different things (gameplay-trigger integrity vs. DLC ownership integrity); a leaked or rotated key for one must never force touching the other, and a second keypair costs nothing to generate. Compile the printed public key into `NEShim.Signing/DlcMapSigner.cs`'s `EmbeddedPublicKeyBase64` constant, and rebuild from source — **this key is deliberately never config-driven**, so it cannot be set via `games/multigame.json` or any `config.json`. Then seal the map: `pub-utils --seal-dlc-map --key-file private_key.txt games/multigame.json` (writes `gameDlcAppIdsSignature` into the manifest in place). Once the embedded key is set, `gameDlcAppIds` must be *complete* — list every game, including bundled ones with value `0` — or that game is rejected; and any signature problem (missing, or the map changed since sealing) rejects *every* game, fail-closed, rather than silently trusting the unsigned data. Re-run `--seal-dlc-map` any time `gameDlcAppIds` changes.
