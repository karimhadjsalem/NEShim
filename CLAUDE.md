...
NEShim's default publish path is single-game: one `config.json` and one ROM next to the exe, matching everything described above. **Multi-game mode is a second, additive publish path on the same binary** — one exe hosting N games, each installed as an independent Steam DLC depot, selected at runtime through a carousel screen. It never changes single-game behavior; it only activates under an explicit, opt-in signal.
...
**Per-game content layout**: each DLC depot contains only a `games/<gameId>/` subfolder (its own ROM, `config.json`, `achievements.json`, artwork) that Steam mounts into the *same* install directory as the base exe — the standard/simplest Steam content-DLC pattern. `steam_appid.txt` and `game_actions_<appid>.vdf` stay singular/app-wide; the whole multi-game install is still one Steam base App ID and one running process. Ownership is checked with `SteamApps.BIsDlcInstalled` (`NEShim/Steam/SteamDlcManager.cs`); a game with `SteamDlcAppId = 0`, or when there's no live Steam session (`SteamManager.IsAvailable == false`), is shown unconditionally — this is also how local dev/test game folders work without configuring real Steamworks entitlements.
...
### Publishing Checklist — multi-game / DLC
...
- **Per-game content**: build each `games/<gameId>/` folder with `local-publish-game.ps1` (`-GameId`, `-SourceDir`, and either `-OutDir` or `-EngineOutDir`) — independent of, and run in addition to, the engine binary publish. One Steam Partner DLC depot per `games/<gameId>/` folder; set that depot's App ID as `steamDlcAppId` in the game's own `config.json`. Set `thumbnailPath` (box art in NES-box aspect ratio, ~1.42:1) and `gameDescription` (shown on the carousel's flip-card back) in the same `config.json` — both are optional; a missing thumbnail shows a placeholder card rather than blocking the entry.
