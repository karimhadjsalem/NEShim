...
**`SDL3HwRenderer` fallback:** When D3D11 initialisation fails, `SDL3HwRenderer` takes over all rendering using SDL_GPU. Structural filters (SPIR-V), color effects, picture adjustments, and three motion effects (CRT Jitter, Scanline Bob, Magnetic Distortion) are fully supported. Video Overlay and Screen Glow (PhosphorPersistence) are not available — both require temporal textures not expressible via SDL_GPURenderState; Screen Glow demotes to None when selected.
...
**`PlatformDetector.IsD3D11Active`** is set once at startup after `D3D11Renderer` is (or isn't) constructed. The in-game Video menu shows a reduced 5-item set when `IsD3D11Active` is false (Window, Filter, Overscan, FPS, Back), hiding Overlay, Motion Effect, Picture, and Presets. `SDL3HwRenderer` supports most of these features internally (SPIR-V shaders) — they are hidden from the menu as a UX simplification but can be configured via `config.json`. Exception: Video Overlay has no effect and Screen Glow (PhosphorPersistence) demotes to None on the SDL_GPU path regardless of the config value.
