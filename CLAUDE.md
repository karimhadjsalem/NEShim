...
**Two-phase frame delivery:** The emulation thread marshals `D3D11Renderer.UploadFrame(FrontBuffer)` to the UI thread via `BeginInvoke` on `MainForm` (the `_uiMarshal` control — not `GamePanel`). The `_steamTimer` (~60 Hz, UI thread) then calls `D3D11Renderer.DrawAndPresent()`, which draws the last uploaded NES texture, composites the overlay texture on top, and presents. This decoupling means Present always fires even when the emulation loop is paused, keeping the Steam overlay hook alive.
