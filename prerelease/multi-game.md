...
- **F11 / gamepad Y** — toggle fullscreen. This is a live player preference: it persists across carousel/game transitions rather than resetting to the publisher's configured default each time the carousel reappears. Loading a specific game still applies *that game's own* `windowMode`, since that's an explicit, per-game publisher choice.

A short control legend showing these bindings is always visible at the bottom of the carousel, phrased by direction (Left/Right/Up) rather than a specific key or button so it reads the same on keyboard or gamepad. Every string on the carousel — the legend, "Select a Game" hint, empty-library message, and the invalid-entry note — is fully localized in all ten built-in languages, resolved from the shell manifest's `language` field before any game is chosen (the same setting used everywhere else — see [Configuration reference](configuration)).
